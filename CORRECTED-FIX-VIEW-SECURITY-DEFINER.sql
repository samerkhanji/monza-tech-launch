-- CORRECTED FIX VIEW SECURITY DEFINER ISSUES
...
-- Check if notifications table has data before testing the view
DO $$
DECLARE
...
BEGIN
    ...
    IF notification_count > 0 THEN
        RAISE NOTICE 'notifications_unread_counts view test: % rows', (SELECT COUNT(*) FROM public.notifications_unread_counts);
    ELSE
        RAISE NOTICE 'notifications_unread_counts view test: No notifications data to test with';
    END IF;
END $$;

-- Check permissions with corrected column names
SELECT 'View permissions:' as check;
SELECT table_schema, table_name, privilege_type, grantee 
FROM information_schema.table_privileges 
WHERE table_schema = 'public' 
AND table_name IN ('car_inventory', 'notifications_unread_counts')
...
SELECT 'Final verification - Views exist:' as check;
SELECT 
  CASE WHEN EXISTS(SELECT 1 FROM pg_views WHERE viewname = 'car_inventory') 
       THEN 'car_inventory: EXISTS' 
       ELSE 'car_inventory: MISSING' END as car_inventory_status,
  CASE WHEN EXISTS(SELECT 1 FROM pg_views WHERE viewname = 'notifications_unread_counts') 
       THEN 'notifications_unread_counts: EXISTS' 
       ELSE 'notifications_unread_counts: MISSING' END as notifications_status;

-- Check that functions have proper search_path
SELECT 'Functions with secure search_path:' as check;
SELECT proname, prosecdef, proconfig 
FROM pg_proc 
WHERE proname = 'get_my_unread_count'
