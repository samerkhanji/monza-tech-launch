...
import { Upload, Plus, Trash2, Check, Wrench } from 'lucide-react';
...
interface IcevBulkUploadDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
const IcevBulkUploadDialog: React.FC<IcevBulkUploadDialogProps> = ({
  isOpen,
  onClose,
  onSave
}) => {
  ...
  const processReceipt = async (imageDataUrl: string) => {
    try {
      ...
      toast({
        ...
        description: `Found ${data.items.length} ICEV parts. Please review and edit as needed.`,
      });
    } catch (error) {
      toast({
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Bulk Upload ICEV Parts
            <Badge variant="outline" className="bg-gray-100 text-gray-800">{step}</Badge>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-auto">
          {step === 'upload' && (
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-gray-700">Upload ICEV Parts Receipt</CardTitle>
                  <CardDescription>
                    Upload receipt for Internal Combustion Engine Vehicle parts. MonzaBot will extract the information automatically.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <CameraSection
          ...
          {step === 'review' && (
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-gray-700">ICEV Order Settings</CardTitle>
                  <CardDescription>
                    These settings will apply to all ICEV parts in this order
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="ICEV parts supplier"
                    />
                  </div>
                  <div className="space-y-2">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center justify-between text-gray-700">
                    ICEV Parts List ({processedItems.length} items)
          ...
          {step === 'review' && (
            <div className="flex gap-2">
              ...
              >
                ...
                Save {processedItems.length} ICEV Parts
              </Button>
            </div>
          )}
        </DialogFooter>
...
export default IcevBulkUploadDialog;
