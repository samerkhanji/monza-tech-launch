...
interface PortalActionDropdownProps {
  ...
  ariaLabel?: string;
  id?: string;
  variant?: 'dots' | 'button'; // dots = ⋯ icon, button = Actions with chevron
  size?: 'sm' | 'md';
}

const PortalActionDropdown: React.FC<PortalActionDropdownProps> = ({
  options,
  onAction,
  className = '',
  disabled = false,
          ...
          <div className="py-1">
            ...
            {groupedOptions.main?.map((option) => (
              <button
                ...
                onMouseDown={(e) => {
                  // Fire action on mousedown so it runs before outside mousedown closes the menu
                  e.stopPropagation();
                  if (!option.disabled) {
